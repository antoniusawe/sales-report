...
if location == "Bali":
    ...
    if bali_option == "Overview":
        ...
        if location_analysis_option == "Occupancy Rate":
            ...
            st.markdown(
            f"<h6 style='text-align: left;'>Site Filled for {previous_month_2}, {previous_month_1}, and {current_month}</h6>",
            unsafe_allow_html=True
            )
...
# Conditional logic based on location selection
    ...
    st.write("Data currently unavailable.")import streamlit as st
import pandas as pd
from streamlit_echarts import st_echarts
from datetime import datetime
from datetime import timedelta
...
if location == "Bali":
    ...
    if bali_option == "Overview":
        ...
        if location_analysis_option == "Occupancy Rate":
            ...
            # Display the filtered table
            st.markdown(
            f"<h4 style='text-align: left;'>Site Filled for {previous_month_2}, {previous_month_1}, and {current_month}</h4>",
            unsafe_allow_html=True
            )
            st.dataframe(filtered_occupancy_summary)
